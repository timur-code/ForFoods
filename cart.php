<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $arrProducts = unserialize(base64_decode($_POST['products']));
        
        for($i = 0; $i < sizeof($arrProducts) - 1; $i++) {
            $product_id = (int)$arrProducts[$i][2];
            $amount = $arrProducts[$i][3];
            $total = $arrProducts[$i][6];
            $insert = "INSERT INTO orders (user_id, product_id, amount, total) VALUES ('$user_id', '$product_id', '$amount', '$total')";
            pg_query($con, $insert);
...
?>
	<div class="wrap cf">
		<h1 class="projTitle">ForFoods<span></span> Shopping Cart</h1>
...
<!--    <ul class="tableHead">
      ...
      <li>Total</li>
       <li>Remove</li>
            ...
            <ul class="cartWrap">
                <?php
                    $products = displayCart($user_data, $con);
                    $total = $products[sizeof($products) - 1];
                    $product = base64_encode(serialize($products));
                ?>
            </ul>
        </div>
